...
public class Version1MasterTests {
    ...
    public void setup() {
        ...
        //for now this this helps make testing easier
//        testGame.clearBoard();
    }
    ...
    public void unionBattleFollowsUnionMove() {
        TestGameEngine test = Factory.makeTestGame();
        test.clearBoard(); //to ensure stacks at start don't trip. todo: make sure stacks are checked in normal conditions
        test.endStep();
        assertEquals(UBATTLE, test.getCurrentStep());
    }
    ...
    public void confederateMoveFollowsUnionBattle() {
        testGame.endStep();
        testGame.endStep();
        assertEquals(CMOVE, testGame.getCurrentStep());
    }
    ...
    public void confederateBattleFollowsConfederateMove() {
        ...
        testGame.endStep();
        testGame.endStep();
        assertEquals(CBATTLE, testGame.endStep());
    }
    ...
    public void hethMovesEast() {
        testGame.endStep();
        testGame.endStep();
        testGame.putUnitAt(heth, 8, 8, EAST); //since clearBoard takes it off
        testGame.moveUnit(heth, makeCoordinate(8, 8), makeCoordinate(10, 8));
        assertEquals(heth, testGame.getUnitsAt(makeCoordinate(10, 8)).iterator().next());
    }
    ...
    public void hethMovesWest() {
        ...
        testGame.endStep();
        testGame.putUnitAt(heth, 8, 8, EAST); //since clearBoard takes it off
        testGame.moveUnit(heth, makeCoordinate(8, 8), makeCoordinate(9, 8));
        assertEquals(heth, testGame.getUnitsAt(makeCoordinate(9, 8)).iterator().next());
    }
    ...
    public void hethMovesAnL() {
        ...
        testGame.moveUnit(heth, makeCoordinate(8, 8), makeCoordinate(9, 6));
        assertEquals(heth, testGame.getUnitsAt(makeCoordinate(9, 6)).iterator().next());
    }
